...
function harness(fetcher=async()=>response(archive)){
  ...
  ids['home-updates-year'].selectedOptions=[{value:'2026',dataset:{version:'20260924-1'}}];
...
test('首頁只保留最新五則，年度舊公告各有唯一完整雙語來源',()=>{
  ...
  assert.deepEqual(dates,['2026-09-24','2026-09-23','2026-09-22','2026-09-18','2026-09-14']);
  ...
  assert.deepEqual(archive.entries.map(entry=>entry.date),['2026-09-08','2026-09-06','2026-08-27','2026-08-25','2026-08-24','2026-08-21','2026-08-20']);
  assert.equal(new Set([...dates,...archive.entries.map(entry=>entry.date)]).size,12);
...
test('初始化零要求，首次載入一次，重複點擊及重看不重複下載或新增列',async()=>{
  ...
  assert.equal(h.ids['home-updates-archive-list'].children.length,7);
  assert.equal(await h.api.show(),true);assert.equal(h.calls.length,1);
  assert.equal(h.ids['home-updates-archive-list'].children.length,7);
  assert.equal(h.calls[0].url,'https://example.test/app/data/home-updates/2026.json?v=20260924-1');
