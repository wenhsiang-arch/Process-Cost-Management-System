...
function harness(fetcher=async()=>response(archive)){
  ...
  ids['home-updates-year'].selectedOptions=[{value:'2026',dataset:{version:'20260925-1'}}];
...
test('首頁只保留最新五則，年度舊公告各有唯一完整雙語來源',()=>{
  ...
  assert.deepEqual(dates,['2026-09-25','2026-09-24','2026-09-23','2026-09-22','2026-09-18']);
  assert.match(html,/重新整理頁面會把訂單數量錯誤顯示為0/);
  assert.match(html,/表格篩選圖示有時會重複出現多個/);
  assert.match(html,/優化款號匯入完成後的結果摘要排版/);
  assert.match(html,/同一個款號內，不可以有兩道完全相同越文名稱的工序/);
  assert.match(html,/原本已經匯入、具有相同越文名稱的舊資料不受影響/);
  assert.match(html,/顯示「產能影響（0）」代表沒有發現差異/);
  assert.match(html,/原產能全部封存後，待處理數量會自動減少/);
  ...
  assert.deepEqual(archive.entries.map(entry=>entry.date),['2026-09-14','2026-09-08','2026-09-06','2026-08-27','2026-08-25','2026-08-24','2026-08-21','2026-08-20']);
  assert.equal(new Set([...dates,...archive.entries.map(entry=>entry.date)]).size,13);
...
test('初始化零要求，首次載入一次，重複點擊及重看不重複下載或新增列',async()=>{
  ...
  assert.equal(h.ids['home-updates-archive-list'].children.length,8);
  assert.equal(await h.api.show(),true);assert.equal(h.calls.length,1);
  assert.equal(h.ids['home-updates-archive-list'].children.length,8);
  assert.equal(h.calls[0].url,'https://example.test/app/data/home-updates/2026.json?v=20260925-1');
